...
def Prepare_Features(DIR,num):
    ...
    data=pd.read_csv(str(DIR)+"/Data/Merck.csv")
...
def runapp(DIR,j,i,n1,n2,n3,batch,lr,seedd,num):
    data=pd.read_csv(str(DIR)+"/Data/Merck.csv")
    ...
    for train_index, test_index in kf.split(UniqList):
        ...
        cb_check = ModelCheckpoint((str(DIR)+'/DNN_CV1_Regression_Cell'+str(j)+'_'+str(FeatureName[i])), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
        ...
        CNN_model = tf.keras.models.load_model((str(DIR)+'/DNN_CV1_Regression_Cell'+str(j)+'_'+str(FeatureName[i])))
    ...
    outFinal.to_csv((str(DIR)+'/DNN_CV1_Regression_Cell'+str(j)+'_'+str(FeatureName[i])+'.csv'))
...
def main():
    ...
    runapp(str(DIR),int(j),int(i),int(n1),int(n2),int(n3),int(batch),float(lr),int(seedd),int(num))
