...
def runapp(DIR,j,i,n1,n2,n3,batch,lr,seedd,num,TINDX):
    ...
    for train_index, test_index in kf.split(UniqList):
        ...
        cb_check = ModelCheckpoint((str(DIR)+'/DNN_CV2_Regression_Cell'+str(j)+'_'+str(FeatureName[i])+'_'+str(TINDX)), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
        ...
        CNN_model = tf.keras.models.load_model((str(DIR)+'/DNN_CV2_Regression_Cell'+str(j)+'_'+str(FeatureName[i])+'_'+str(TINDX)))
    ...
    outFinal.to_csv(str(DIR)+'/DNN_CV2_Regression_Cell'+str(j)+'_'+str(FeatureName[i])+'_'+str(TINDX)+'.csv')
