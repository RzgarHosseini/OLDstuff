...
def runapp(DIR,j,i,n1,n2,n3,batch,lr,seedd,num):
    ...
    for train_index, test_index in kf.split(UniqList):
        ...
        cb_check = ModelCheckpoint((str(DIR)+'/DNN_CV1_Classification_Cell'+str(j)+'_'+str(FeatureName[i])), verbose=1, monitor='val_loss',save_best_only=True, mode='auto')
        ...
        CNN_model = tf.keras.models.load_model(str(DIR)+'/DNN_CV1_Classification_Cell'+str(j)+'_'+str(FeatureName[i]))
    ...
    outFinal.to_csv((str(DIR)+'/DNN_CV1_Classification_Cell'+str(j)+'_'+str(FeatureName[i])+'.csv'))
